...
*   **Управляющий хост Ansible:** Linux A (`192.168.31.108`, пользователь `sheptunov_1`)
*   **Хост приложения (Todo-app):** Linux B (`192.168.31.48`, пользователь `sheptunov_2`), далее `client` в Ansible.
*   **Хост базы данных (MySQL):** Linux C (`192.168.31.110`, пользователь `sheptunov_3`), далее `server` в Ansible.
...
1.  **Создание структуры проекта:**
    ...
    *Структура папок представлена в `folder_structure.txt`.*
...
2.  **Настройка инвентаря (`inventory.ini`):**
    ...
    [app]
    client ansible_host=192.168.31.48 ansible_user=sheptunov_2

    [db]
    server ansible_host=192.168.31.110 ansible_user=sheptunov_3
    ```
...
4.  **Настройка целевых хостов (Linux B, Linux C):**
    ...
    *   Для пользователей `sheptunov_2` (на Linux B) и `sheptunov_3` (на Linux C) настроен беспарольный `sudo` для выполнения задач Ansible с повышением прав (`become: yes`).
...
*   **Запуск контейнера Todo-app:**
    ...
    *   Порт `80` контейнера (на котором слушает приложение) пробрасывается на порт `3002` хоста.
    *   Переменные окружения для подключения к базе данных (`DB_HOST`, `DB_USER`, `DB_PASSWORD`, `DB_NAME`) передаются в контейнер. `DB_HOST` указывает на IP-адрес хоста `server` (`192.168.31.110`), а `DB_PASSWORD` берется из `vault.yml`.
...
*   **Контейнер Todo-app (на Linux B):** Успешно запущен, порт `3002` доступен извне.
*   **Веб-приложение:** Приложение Todo-app доступно в браузере по адресу `http://192.168.31.48:3002`. Функциональность создания, редактирования и удаления задач проверена и работает корректно, что подтверждает успешное подключение к базе данных MySQL.
